...
void FriendlyTalkerView::set_tx(bool tx) {
    if (tx) {
        ...
        transmitter_model.set_baseband_bandwidth(1750000); // Standard for NFM
        transmitter_model.set_rf_amp(true);
        transmitter_model.set_lna(40);
        transmitter_model.set_vga(40);
        
        // Microphone and Audio setup
        audio::input::start(); // Start ADC for earphone/internal mic
...
void FriendlyTalkerView::update_vumeter() {
    // In a real app, we would poll RSSI from the receiver model
    if (!transmitting) {
        float rssi = receiver_model.level(); // 0.0 to 1.0
